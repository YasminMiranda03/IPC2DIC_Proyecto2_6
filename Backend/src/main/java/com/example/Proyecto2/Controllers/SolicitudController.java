...
public class SolicitudController {
    ...
    public ResponseEntity<?> eliminarSolicitud(@PathVariable("id") String id) {
        ...
        if (!eliminado) return ResponseEntity.status(404).body("ERROR: Solicitud no encontrada: " + id);
    ...
    public ResponseEntity<RespuestasProcesamiento> procesarN(@PathVariable("n") int n) {
